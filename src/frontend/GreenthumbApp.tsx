...
import { GreenthumbAPI } from "./services/greenthumb-api";
...
const GreenthumbApp: React.FC = () => {
  ...
  const [hasAttemptedWalletCreation, setHasAttemptedWalletCreation] =
  ...
  const AGENT_WALLET_ADDRESS =
    ...
    "0x742d35Cc6634C0532925a3b8D591D3D5e0cc3b1c"; // fallback for demo
  ...
  // Create API instance lazily
  const getAPI = () => {
    if (!api.current) {
      console.log(`⚡ GreenThumb: Creating API instance`);
  ...
  useEffect(() => {
    ...
    const checkConnection = async () => {
      console.log(`🏥 GreenThumb: Health check`);
      try {
        const healthOk = await getAPI().healthCheck();
        setIsConnected(healthOk);
        console.log(
          healthOk
            ? `✅ GreenThumb: Edge Function is healthy`
            : `❌ GreenThumb: Edge Function health check failed`,
        );
      } catch (error) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      console.log("📤 GreenThumb: Sending message:", messageText);
      const response = await getAPI().sendMessage(messageText);

      console.log("📨 GreenThumb: Received response:", response);
  ...
  const checkUSDCBalance = async () => {
    ...
    try {
      ...
      const balanceData =
        "0x70a08231" + embeddedWallet.address.slice(2).padStart(64, "0");
  ...
  const fundWithUSDC = async () => {
    ...
    try {
      ...
      alert(
        "Failed to fund wallet. This feature may not be available on testnet.",
      );
    }
  };
            ...
            >
              ...
              <div
                style={{
                  ...
                  justifyContent: "center",
                  backgroundColor:
                    message.sender === "bot" ? "#10b981" : "#6b7280",
                  color: "white",
                  flexShrink: 0,
                }}
              >
                  ...
                  style={{
                    ...
                    borderRadius:
                      message.sender === "user"
                        ...
                        : "18px 18px 18px 6px",
                    backgroundColor:
                      message.sender === "user" ? "#10b981" : "white",
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              border: "none",
              backgroundColor:
                inputText.trim() && isConnected ? "#10b981" : "#d1d5db",
              color: "white",
              cursor:
                inputText.trim() && isConnected ? "pointer" : "not-allowed",
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              boxShadow: "0 2px 8px rgba(0, 0, 0, 0.1)",
          ...
          >
            ...
            <div style={{ display: "flex", gap: "12px" }}>
              ...
              <button
                onClick={sendTip}
                disabled={
                  isSendingTip || !tipAmount || parseFloat(tipAmount) <= 0
                }
                style={{
                  flex: 1,
                  padding: "12px",
